...
export default function Header() {
  ...
  return (
    <header className="h-14 border-b border-white/[0.04] bg-[#070a14]/80 backdrop-blur-md flex items-center gap-4 px-6 flex-shrink-0 z-10">
      <button
      ...
      <div className="flex-1 max-w-md hidden sm:block" ref={searchRef}>
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="search cameras, events, rules..."
            value={query}
            onChange={(e) => { setQuery(e.target.value); setShowResults(true); }}
            onFocus={() => setShowResults(true)}
            className="w-full bg-[#0a0c14]/60 border border-white/[0.04] rounded-lg pl-9 pr-4 py-1.5 text-sm text-slate-300 placeholder:text-slate-600 focus:outline-none focus:border-[#7eb8f7]/30 focus:bg-[#0c1020]/90 transition-all font-mono-data lowercase"
          />
          {showResults && hasResults && (
            <div className="absolute top-full left-0 right-0 mt-2 bg-[#0a0c14] border border-white/[0.04] rounded-lg shadow-xl z-50 overflow-hidden backdrop-blur-lg">
              {camResults.length > 0 && (
                <div className="px-3 py-1.5 text-[10px] font-semibold text-slate-500 uppercase tracking-wider border-b border-white/[0.04] font-mono-data">cameras</div>
              )}
              {camResults.map((c) => (
                <button key={c.id} onClick={() => navigate('/cameras')} className="w-full text-left px-3 py-2 text-xs text-slate-300 hover:bg-white/[0.03] truncate font-mono-data lowercase">
                  {c.name}
                </button>
              ))}
              {alertResults.length > 0 && (
                <div className="px-3 py-1.5 text-[10px] font-semibold text-slate-500 uppercase tracking-wider border-b border-white/[0.04] border-t font-mono-data">alerts</div>
              )}
              {alertResults.map((a) => (
                <button key={a.id} onClick={() => navigate('/alerts')} className="w-full text-left px-3 py-2 text-xs text-slate-300 hover:bg-white/[0.03] truncate font-mono-data lowercase">
                  {a.alert_type.replace(/_/g, ' ')}
                </button>
              ))}
              {detResults.length > 0 && (
                <div className="px-3 py-1.5 text-[10px] font-semibold text-slate-500 uppercase tracking-wider border-b border-white/[0.04] border-t font-mono-data">detections</div>
              )}
              {detResults.map((d) => (
                <button key={d.id} onClick={() => navigate('/events')} className="w-full text-left px-3 py-2 text-xs text-slate-300 hover:bg-white/[0.03] truncate font-mono-data lowercase">
                  {d.object_type}
                </button>
              ))}
            </div>
